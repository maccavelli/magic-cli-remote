...
class _SessionsScreenState extends ConsumerState<SessionsScreen>
  ...
  Future<String?> _createSessionFlow(McremoteClient client) async {
    ...
    // Futures are inserted before they are awaited, so prefetch and a user tap
    // share one host request. Successful results remain cached for this dialog.
      ...
      builder: (ctx) {
        return StatefulBuilder(
          builder: (ctx, setModal) {
            // Cache the future itself so concurrent callers share success or
            // failure. A failed entry is evicted before its scoped fallback is
            // returned, allowing the next explicit action to retry.
            Future<PickerCatalog> catalogFor<K>(
              Map<K, Future<PickerCatalog>> cache,
              K key,
              String label,
              Future<PickerCatalog> Function() fetch,
              PickerCatalog Function() fallback,
            ) {
              final hit = cache[key];
              if (hit != null) return hit;
              late final Future<PickerCatalog> pending;
              pending = Future<PickerCatalog>.sync(fetch).catchError((
                Object e,
              ) {
            ...
            Future<PickerCatalog> loadModelProviders(String p) {
              return catalogFor(
                providerCatalogs,
                p,
                'providers',
                () => client.listModels(p, scope: 'providers'),
                () => PickerCatalog(provider: p, allowCustom: false),
              );
            }

            Future<PickerCatalog> loadModels(String p, String mp) {
              final key = (p, mp);
              return catalogFor(
                modelCatalogs,
                key,
            ...
            Future<void> pickModel() async {
              ...
              final providerCatalog = await loadModelProviders(p);
              if (!ctx.mounted || provider != p) return;
              final result = await showModelPicker(
                ctx,
                provider: p,
                providerCatalog: providerCatalog,
                loadModels: (mp) => loadModels(p, mp),
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        InputDecorator(
                          ...
                          child: InkWell(
                            onTap: provider == null ? null : pickModel,
                            child: Row(
                              children: [
                                Expanded(
                                  child: Text(
  ...
  String _connLabel(
    ...
    // Naming the transport is half the fix: since 0062 a session runs over
    // mesh or relay, and "Connected to host" could not tell a healthy relay
    // failover from a mesh that died 40 seconds ago (MADR 0063 D5).
    final via = transport == null ? '' : ' over ${transport.label}';
    switch (s) {
      case McConnectionState.connected:
        if (health == LinkHealth.stale) {
          return 'Checking connection to $hostname$via…';
        }
        if (health == LinkHealth.lost) {
          return transport == null
              ? 'Connection lost — retrying…'
              : '${transport.label} connection lost — retrying…';
        }
        return 'Connected to $hostname$via';
  ...
  Widget build(BuildContext context) {
    ...
    final healthClient = ref.watch(mcremoteClientProvider);
    final activeTransport = healthClient.activeTransport;
    // Green is a claim that expires: `connected` alone only means a socket was
    // adopted and authenticated, which stays true across a blackholed path
    // (MADR 0063 D1). `degraded` keeps the strip visible but stops it lying.
    final connected = connState == McConnectionState.connected;
