...
| F6 (F6g) | Ten rapid background/foreground cycles against a live daemon (both transports; on relay, watch `mcrelay` logs) | No `too many clients` from the daemon, no `limit`/`rate_limited` from the relay, session intact after the tenth resume; answers 0067 A1-Q5 (how long a suspended socket lingers server-side) | ⏸ no device |
