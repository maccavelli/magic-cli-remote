...
| normal versus experimental schema | Normal 0.147 exports `thread/fork`, all three goal methods, `review/start`, `model/list`, `gitDiffToRemote`, and `turn/start.{serviceTier,personality}`. Only `thread/settings/update` and collaboration fields/methods require experimental opt-in. |
...
| live working-tree diff | Non-experimental `gitDiffToRemote {cwd}` returned a SHA equal to repository `HEAD` and a 43,808-byte unified patch containing the then-untracked MADR 0080. This confirms untracked-file inclusion on this release. |
