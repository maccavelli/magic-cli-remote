...
- [x] Complete P2 callback correctness, secret forms, and provider routing.
- [x] Complete P3 managed transports, supervision, and reconnection.
- [x] Complete P4 event fidelity, runtime status, and diagnostics.
- [x] Complete P5 permission profiles, reviewer axis, and managed settings.
- [x] Complete P6 unified managed/native thread browser and sections.
- [x] Complete P7 sandboxed execution, explicit shell, and terminals.
...
| P2 | `b51c196` | Exact focused Codex callback/form/secret/approval/permission/routing/resolution tests, event/protocol/session/WS packages, repository-wide `go test ./...`, race tests for Codex/session/WS, Dart format, Flutter analyze, and the question/provider-auth widget suites passed 2026-08-25; `make pre-add-check` reported all 22 changed Go files clean (the unreachable vulnerability database was a warning under the repository gate) | All 75 stable notifications have an explicit provider/session destination; provider-global rate-limit and MCP startup events no longer depend on a thread id; callback records bind exact response dialect and engine generation; granular responses carry requested-subset `permissions`, `turn`/`session` scope, and both strict-review states without a `decision` field | Added keyed answer compatibility from phone through providers, exact 0.149.1 question/approval fixtures, authoritative `serverRequest/resolved` cleanup, a widget-local secret editor with redacted Go `LogValuer` implementations and post-dispatch clearing, the single Codex-specific phone-operation registry, and additive protocol documentation. The Flutter tool resolved six transitive versions downward under its bundled SDK during verification; the unrelated lockfile rewrite was intentionally excluded. |
| P3 | `b56b00f` | Shared stdio/Unix-WS/TCP-WS conformance and focused launch/reconnect/replacement/proxy tests, config/daemon/WS packages, repository-wide `go test ./...`, and the prescribed Codex/daemon/WS race suites passed 2026-08-25; `make pre-add-check` reported all 21 changed Go files clean (the unreachable vulnerability database was a warning under the repository gate) | stdio remains the default; Unix sockets are created below a `0700` daemon runtime directory and chmod `0600`; TCP is loopback-only with generated capability-token or signed-bearer native auth, `/readyz` plus `/healthz`, and authenticated initialize; raw Codex JSON-RPC is rejected by the typed phone boundary; `-32001` retries only read-only calls | Added transport-owned framing with conn-owned correlation, reserved-port launch, bounded readiness and cleanup, generation replacement with one loaded-thread reconciliation and stale-callback cancellation, three bounded backoffs, exact single-object managed-daemon lifecycle parsing, `started`-only leases, foreign-daemon refusal, verified stop, and stdio fallback after proxy lease loss. No socket, port, or bearer is exposed to phones; no push occurred. |
| P4 | `2ca5f5a` | Focused item/event/runtime/status/diagnostic/rate/warning/Guardian/model tests, event/protocol/session/WS packages, repository-wide `go test ./...`, the prescribed Codex/event/WS race suites, Flutter analyze plus Codex-runtime/transcript-reducer tests, Dart format, and `git diff --check` passed 2026-08-25; `make pre-add-check` reported all 32 changed Go files clean (the unreachable vulnerability database was a warning under the repository gate) | Negotiated Codex surface version 1 exposes only a sorted, bounded capability projection; lifecycle events use stable upsert keys; runtime reads are typed and bounded; unknown completion-relevant items become sanitized unsupported cards; doctor accepts only schema 1, is single-flight with a 30-second/256-KiB bound, strips paths, URLs, secrets, and remediation, and persists nothing | Added the missing additive v2 Codex-surface offer/caps gate assigned to P2, provider-global account/rate/usage/model/config/feature/MCP snapshots, `/status` and `/usage`, readable reasoning/progress/terminal/patch/Guardian/reroute/verification/safety/memory events, and an explicit mobile Runtime screen with Doctor action. The Flutter tool's unrelated lockfile rewrite was restored and excluded. No push occurred. |
| P5 | `8889c1c` | Focused permission-profile/reviewer/Guardian/settings/provenance tests, command/session/protocol/WS packages, repository-wide `go test ./...`, the prescribed Codex/session race suites, Flutter analyze plus `codex_permissions_test.dart`, Dart format, and `git diff --check` passed 2026-08-25; `make pre-add-check` reported all 29 changed Go files clean (the unreachable vulnerability database was a warning under the repository gate) | Installed 0.149.1 read-only probes pinned the native built-ins `:read-only`, `:workspace`, and `:danger-full-access`; missing catalog support falls back to the legacy four modes. Profile, sandbox, reviewer, and unattended axes are independently tested; managed profile/approval restrictions remove disallowed choices while preserving an explanatory sanitized policy label. Versioned `config/batchWrite` uses one atomic edit set with hot reload and fixed keys only | Added additive profile/reviewer session metadata and old-record compatibility, authoritative settings reconciliation, `/permissions`, independent `/reviewer`, exact one-shot `/approve`, provider-global requested/effective provenance, negotiated `codex.permissions.write`, and Runtime controls with dangerous confirmation. Only ids, reviewer state, sanitized layer classes, and versions persist or cross the phone boundary; policy documents, config values, developer instructions, denial payloads, and paths do not. The Flutter tool's unrelated lockfile rewrite was restored and excluded. No push occurred. |
| P6 | `f2b8e48` | The prescribed thread/section/search/archive/delete/replay/reconcile tests, all project behavioral fixtures, session/protocol/WS packages, the Codex/session/WS race suites, Flutter analyze plus `codex_threads_test.dart` and `sessions_screen_test.dart`, Dart format, repository-wide `go test ./...`, and `git diff --check` passed 2026-08-25; `make pre-add-check` reported all 27 changed Go files clean (the unreachable vulnerability database was a warning under the repository gate) | Stable list/read/loaded-list/lifecycle/section operations remain available independently of experimental native search and turn/item cursors; stable search fallback is labelled and bounded to 20 pages/1000 rows. Project list sends `limit:50`, pins upstream default 25/maximum 100, and stops at 20 pages/1000 projects. Missing project capabilities fall back to ungrouped threads; ambiguous mutations reconcile through native reads | Added a unified managed/native Threads screen, additive aliases with exact-id preference and loaded-thread adoption, cold replay without duplicate live broadcasts, complete section/project organization, fixed `/archive` and `/delete` confirmations with descendant/partial results, seven project methods, and typed Codex read/write protocol operations. Project deletion preserves threads and roots; section deletion preserves and unsections members. Flutter's unrelated transitive lockfile rewrite was restored and excluded. No push occurred. |
| P7 | `fbee64f` | The prescribed `Test(Shell|Exec|Terminal|Output|PTY|Environment|Process)` suite, command/session/protocol/ws/config/daemon packages, repository-wide `go test ./...`, the Codex/session/WS race suites, `flutter analyze` over lib and test, `dart format`, the full 1070-test mobile suite including the new `codex_execution_test.dart`, and `git diff --check` passed 2026-08-25; `make pre-add-check` reported all 613 files clean (the unreachable vulnerability database was a warning under the repository gate) | Stable `command/exec` and `thread/shellCommand` are available independently of experimental background terminals, environments, and `process/*`; a missing native terminal catalog returns `native_unavailable` and leaves daemon-owned terminals working. Standalone processes stay default-off behind `standalone_processes_enabled` with a host-only environment-name allowlist that rejects secret-like names at config load. `ws://` environments are loopback-only; every other host requires `wss://` | Added the daemon terminal registry (1 MiB per terminal, generation-scoped), `codex.execution.read`/`.write`, the `codex.terminal.output` live push to the owning device only, `/ps` and strict `/stop`, exact `!confirm <command>` gating, session-manager execution delegation, and a mobile terminal view. Terminal bytes deliberately bypass `event.Event` so they never enter session history or the retained ring; `sequence_gap` reports a dropped replay position rather than splicing output. `outcome_unknown` is never auto-retried. Stdin and environment values are redacted from both log surfaces and cleared after dispatch. The Flutter tool's unrelated transitive lockfile rewrite was restored and excluded, as were the pre-existing process-rule worktree changes. No push occurred. |
