...
| **F3** | The `goose` binary is ad-hoc, linker-signed, with no team identity. | `codesign -dv` on `/Users/<user>/.local/bin/goose` reports `Signature=adhoc`, `flags=0x20002(adhoc,linker-signed)`, `TeamIdentifier=not set`, and a per-build `Identifier=goose-9212d893d42becc0`. | Confirmed by probe. |
...
| **F6** | The daemon runs as a user LaunchAgent inside the GUI session. | `com.magiccliremote.mcremote.plist` is in `~/Library/LaunchAgents`, and `launchctl list` shows the label with PID 81345 running as `<user>`. This is why a prompt can appear at all: a LaunchDaemon could not reach the login keychain. | Confirmed by probe. |
