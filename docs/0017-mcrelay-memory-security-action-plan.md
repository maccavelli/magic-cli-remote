...
- **Status**: Implemented (A–D + E1–E3; E5 still deferred)
- **Date**: 2026-07-23
...
| **D14** | Trusted proxies | **Originally defer; pulled forward as E1**: default still `RemoteAddr` only; opt-in `trusted_proxies` CIDRs honor XFF/X-Real-IP only from those hops |
...
### Phase E1 — Trusted proxies (P3, **shipped**)

**Goal:** Correct per-client rate limits behind a reverse proxy without opening
XFF spoofing on the public edge.

| Step | Work | Status |
|------|------|--------|
| **E1.1** | `trusted_proxies` YAML/env; parse CIDR or bare IP | **Done** |
| **E1.2** | `clientIP`: ignore XFF unless `RemoteAddr` ∈ trusted; rightmost non-trusted hop | **Done** |
| **E1.3** | Tests: spoof blocked, trusted path + rate buckets | **Done** |
| **E1.4** | Docs + example config | **Done** |

PROXY protocol is **not** implemented (ops can terminate at LB that sets XFF).
...
| 2026-07-23 | **E1:** opt-in `trusted_proxies` + safe XFF/X-Real-IP client IP for rate limits; default still RemoteAddr-only. E5 still deferred. |
