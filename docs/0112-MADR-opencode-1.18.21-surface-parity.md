...
| **A13** | Unit tests are part of every production change, not a cleanup phase. Every phase that changes production code ships its tests in the same commit, covering success, sanitized failure, boundary, compatibility, and applicable race/cancellation paths. Every **new** Dart production file this plan creates reaches at least 90.0% line coverage. For every existing target a phase touches, that phase must not make it worse: the exact covered/total fraction may not fall, and at least one touched executable target must strictly improve. The absolute uncovered count is reported but is **not** a gate (amended 2026-08-23): a phase that adds new surface necessarily raises it — P2 added 53 lines to `sessions_screen.dart` and covered 38, which is better than that file's existing average and still added 15 uncovered lines. Failing on the count would penalise adding well-tested code, so the fraction is the rule. Live, token-bearing, loopback, and end-to-end tests supplement but never count toward these unit numbers. |
