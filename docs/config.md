...
| `providers.grok.reasoning_effort` | *(empty — pass `--reasoning-effort <EFFORT>` to `grok agent` when non-empty. Live models advertise the set: grok-4.6 is `low`, `medium`, `high`, `xhigh`; grok-4.5 is `low`, `medium`, `high`)* |
