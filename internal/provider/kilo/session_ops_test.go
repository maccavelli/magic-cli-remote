...
// route is a canned (path fragment, body) pair a recorder answers with.
...
// (/api/session/{id}/compact) answered 503 "not available yet" on opencode
// 1.18.5, and Kilo's OpenAPI dump inherits the same route split (MADR 0075
// §2.4) — so this must stay on the legacy summarize endpoint (MADR 0023).
...
// A bare name has no fork-wide "Zen" concept on Kilo — splitModel's fallback
// provider for a slash-less name is "kilo" (Kilo Gateway), matching session
// create (MADR 0076 plan P8: opencode's TestSetModelTreatsBareNameAsZen ported
// with the expected provider adjusted; the underlying splitModel/SetModel
// mechanism is identical, only the fallback provider literal differs).
func TestSetModelTreatsBareNameAsKiloGateway(t *testing.T) {
	h := newRecorder()
	s := newOpsSession(h)
	...
	if ref["providerID"] != "kilo" || ref["id"] != "grok-code" {
		t.Fatalf("ref=%+v want the kilo gateway provider", ref)
	}
}
...
// Kilo streams no usage of its own: /context and the client's context
...
// spawns and polls. Changing one silently breaks every Kilo session.
