...
const busyFrame = `{"sessionID":"ses_test","status":{"type":"busy"}}`
...
func TestBusyAfterIdleAnnouncesRunningAgain(t *testing.T) {
	...
	s.HandleEvent("session.status", json.RawMessage(busyFrame))
	s.HandleEvent("session.status", json.RawMessage(busyFrame))
	s.HandleEvent("session.status", json.RawMessage(`{"sessionID":"ses_test","status":{"type":"idle"}}`))
...
func TestErrorStatusReopensTheLatch(t *testing.T) {
	...
	s.HandleEvent("session.error", json.RawMessage(
		`{"sessionID":"ses_test","error":{"name":"BoomError","data":{"message":"boom"}}}`))
	s.HandleEvent("session.status", json.RawMessage(busyFrame))
...
// client's batch window. Kilo ships message.updated repeatedly per turn,
// usually with identical counts (same OpenCode-lineage engine behavior).
