...
// TestIsControlPlan (opencode's todo_test.go) tests a shared internal/event
// invariant (event.IsControl(event.TypePlan)), not kilo-specific code — it is
// already covered by opencode's own copy of the same test and is not ported
// here (MADR 0076 plan P8, D4).

func TestMapKiloTodos(t *testing.T) {
	got := mapKiloTodos([]kiloTodo{
...
func TestMapKiloTodosEmptyClear(t *testing.T) {
	got := mapKiloTodos(nil)
	if got == nil {
...
// resyncTreeTestHost adds NoteNodeStatus tracking and a turnOn-respecting
// EndTurn on top of captureHost — enough for TestResyncTodosWhileTreeBusy,
// which exercises resync.go's tree-busy detection (discoverTreeChildren /
// resyncTreeState). That REST-based busy check runs unconditionally of the
// session_tree config flag (only child-alias *binding* is gated — MADR 0076
// plan P4 finding), so this is currently-active production logic, not a
// dormant tree feature deferred with session_tree (MADR 0075 PD2/Q7).
type resyncTreeTestHost struct {
	captureHost
	nodes  map[string]httpagent.NodeStatus
	turnOn bool
}

func newResyncTreeTestHost() *resyncTreeTestHost {
	return &resyncTreeTestHost{nodes: map[string]httpagent.NodeStatus{}, turnOn: true}
}

func (h *resyncTreeTestHost) NoteNodeStatus(id string, st httpagent.NodeStatus) {
	h.mu.Lock()
	if id == "" {
		id = h.AgentSessionID()
	}
...
func (h *resyncTreeTestHost) EndTurn() bool {
	h.mu.Lock()
	defer h.mu.Unlock()
	if !h.turnOn {
		return false
...
func TestResyncTodosWhileTreeBusy(t *testing.T) {
	...
	h.api = func(_ context.Context, method, path string, _, out any) error {
		...
		case strings.Contains(path, "/session/status"):
			return json.Unmarshal([]byte(`{"ses_test":{"type":"idle"},"child1":{"type":"busy"}}`), out)
