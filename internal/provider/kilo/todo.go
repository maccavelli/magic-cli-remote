...
// kiloTodo is the Kilo engine Todo shape (OpenCode SDK 1.18+ lineage).
type kiloTodo struct {
	ID       string `json:"id"`
...
func (o *httpSession) handleTodoUpdated(props json.RawMessage) {
	...
	_ = p.SessionID
	entries := mapKiloTodos(p.Todos)
	o.h.Emit(event.Event{Type: event.TypePlan, Entries: entries})
}

// mapKiloTodos converts Kilo todos to plan entries (Q4 cancelled mapping).
// Preserves engine order; drops empty content; never marks cancelled as completed.
func mapKiloTodos(todos []kiloTodo) []event.PlanEntry {
