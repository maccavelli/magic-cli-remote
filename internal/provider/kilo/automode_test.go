...
// TestAutoModeIsAdvertisedLast pins the ordering session.defaultMode depends
// on: it resolves "return to normal" as code (Kilo's default agent — it has
// no `build`, MADR 0076 M1), else the first non-plan mode. Auto anywhere
// earlier would let `/plan off` drop the user into auto-approve.
func TestAutoModeIsAdvertisedLast(t *testing.T) {
	t.Run("static", func(t *testing.T) {
		...
		if got[0] != "code" {
			t.Errorf("static modes = %v, want code first so /plan off resolves to it", got)
		}
	})
	...
	t.Run("from_agent_catalog", func(t *testing.T) {
		...
		want := []string{"code", "plan", "reviewer", "auto"}
...
func TestSetModeAutoArmsAndKeepsNormalAgent(t *testing.T) {
	...
	if agent := h.Agent(); agent != "code" {
		t.Fatalf("agent = %q, want code; auto must not be set as an agent name", agent)
	}
}

func TestSetModeAwayFromAutoDisarms(t *testing.T) {
	for _, target := range []string{"code", "plan"} {
...
// TestCurrentModeReportsAutoWhenArmed: the chip must never read "code" on a
...
func TestCurrentModeReportsAutoWhenArmed(t *testing.T) {
	...
	h.SetAgent("code")
	if got := s.currentMode(modes); got != "code" {
		t.Fatalf("currentMode = %q, want code when auto is off", got)
	}
