...
// was live-probed against (MADR 0075, docs/kilo-spike-7.4.20/). Kilo is a
// fast-moving OpenCode fork, so a version drifting from this pin is worth a
// log line even though nothing refuses to run yet — a minimum-version gate
// only becomes real when session_tree needs one (plan PD2 / MADR Q7).
const KnownGoodVersion = "7.4.20"

// OnHealthy implements [httpagent.HealthyHook]: records the engine version
// from GET /global/health ({"healthy":true,"version":"7.4.20"} on the spike
// host) for doctor output and future gating.
