...
// model flag (matching pre-refactor behavior).
func TestSpecModelArgs(t *testing.T) {
	got := spec.ModelArgs(Config{AlwaysApprove: true, Args: []string{"custom"}}, "m2")
	want := []string{"agent", "--no-leader", "--always-approve", "-m", "m2", "stdio"}
	if len(got) != len(want) {
		t.Fatalf("args = %v", got)
	}
	for i := range want {
