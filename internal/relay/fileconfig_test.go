...
// isolateRelay makes Load(LoadOptions{}) hermetic (MADR 0162 D2): every
// platform root points into a fresh temp dir, and every MCRELAY_* variable
...
// XDG_CONFIG_HOME — and a stray MCRELAY_CONFIG overrides even that.
//
...
func isolateRelay(t *testing.T) string {
	t.Helper()
	d := t.TempDir()
	roots := appdirs.Roots{
		Home:        d,
	...
	prev := systemRoots
	systemRoots = func(appdirs.Product) (appdirs.Roots, []appdirs.Diagnostic, error) {
		return roots, nil, nil
	}
	t.Cleanup(func() { systemRoots = prev })
	for _, kv := range os.Environ() {
		// Viper treats an empty value as unset, and Load ignores an empty
		// MCRELAY_CONFIG.
		if name, _, _ := strings.Cut(kv, "="); strings.HasPrefix(name, "MCRELAY_") {
			t.Setenv(name, "")
		}
	}
	dir := filepath.Join(d, "config", "mcrelay")
	paths, err := appdirs.Resolve(appdirs.ProductMcrelay, roots, "")
	if err != nil {
...
// TestIsolatedRelayLoadReadsOnlyTheIsolatedConfig proves the seam is wired
// (MADR 0162 D4): if Load stopped resolving roots through systemRoots, it
// would read the host's config instead of this one and fail here.
func TestIsolatedRelayLoadReadsOnlyTheIsolatedConfig(t *testing.T) {
	dir := isolateRelay(t)
	// A host entry carries a secret, so the directory must be private or the
	// credential guard refuses the file (same as privateFixtureDir).
	...
	body := `hosts:
  - id: isolated-host
    secret: sixteen-chars-min-i
`
	if err := os.WriteFile(want, []byte(body), 0o600); err != nil {
		t.Fatal(err)
	}
	cfg, err := Load(LoadOptions{})
	if err != nil {
	...
	if cfg.ConfigFile != want {
		t.Fatalf("ConfigFile = %q, want %q: Load did not read the isolated config", cfg.ConfigFile, want)
	}
	found := false
	for _, h := range cfg.Hosts {
		if h.ID == "isolated-host" {
			found = true
		}
	}
	if !found {
		t.Fatalf("isolated host missing from %+v", cfg.Hosts)
	}
}
